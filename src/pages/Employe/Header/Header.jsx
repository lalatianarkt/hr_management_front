...
export default function Header() {
  ...
  const getNomComplet = () => {
    try {
      ...
      // console.log("response.data.infosPro:", sessionStorage.getItem('idEmploye'));
  ...
  const getNomDepartement = () => {
    try {
      const departement = sessionStorage.getItem('departement') || sessionStorage.getItem('departement');
      if (departement) return departement;

      const userStr = sessionStorage.getItem('user');
      if (userStr) {
        const user = JSON.parse(userStr);
        return user.departement?.nom || user.infosPro?.departement?.nom || user.infosPro?.poste?.departement?.nom || 'Departement';
      }

      return 'Departement';
    } catch (error) {
      console.error('Erreur lecture nom departement:', error);
      return 'Departement';
    }
  };
  ...
  const getBreadcrumbItems = (pathname) => {
    const routes = [
      { match: /^\/emp\/?$/, items: ["Accueil"] },
      { match: /^\/emp\/infos\/fiche-perso$/, items: ["Accueil", "Personnel", "Fiche personnelle"] },
      { match: /^\/emp\/presence\/pointage$/, items: ["Accueil", "Personnel", "Pointage"] },
      { match: /^\/emp\/conge\/demande$/, items: ["Accueil", "Demandes", "Demande conge"] },
      { match: /^\/emp\/absence\/demande$/, items: ["Accueil", "Demandes", "Demande absence"] },
      { match: /^\/emp\/absence\/liste\/demande$/, items: ["Accueil", "Demandes", "Mes absences"] },
      { match: /^\/emp\/mouvement\/demande$/, items: ["Accueil", "Demandes", "Demande mouvement"] },
    ];
            ...
            <div className="m-0 small text-muted" style={{ fontSize: "10px" }}>
              {breadcrumbItems.map((item, index) => (
                <React.Fragment key={`${item}-${index}`}>
                  {index === 0 ? (
                    <Link to="/emp" className="text-decoration-none" style={{ color: "var(--color-text-muted)" }}>
                      {item}
                    </Link>
                  ) : (
                    <span>{item}</span>
              ...
              <span className="ms-2">| Departement : {getNomDepartement()}</span>
            </div>
          </div>
        </div>
            ...
            {showNotifications && (
              ...
              >
                ...
                <div className="p-2 border-top text-center">
                  <Link to="/notifications" className="text-decoration-none small" style={{ color: "var(--bg-primary)" }}>
