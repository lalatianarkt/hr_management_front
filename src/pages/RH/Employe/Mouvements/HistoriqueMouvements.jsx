...
const handleRejectMouvement = async (mouvementId) => {
  ...
  // NOUVEAU FLUX : Gestion des étapes pour l'ajout de mouvement
  const handleAddMouvement = () => {
    setSelectedMouvement(null);
    setSelectedEmployee(null);
    setSelectedEmployeForMouvement(null);
    setSelectedTypeMouvement(null);
    ...
    // Étape 2: Sélection du type de mouvement
    const handleSelectTypeMouvement = (type) => {
    console.log("Type sélectionné (objet complet):", type); // Vérifiez que vous avez bien l'id
    setSelectedTypeMouvement(type);
        // console.log("ato nefa ah", type.id);
        console.log("ato nefa ah", type.value);
        // console.log("Employe details:", employeDetails.infosProfessionnelles.poste.id); // ⬅️ AJOUTEZ CE LOG
    
    
    // Pré-remplir les champs selon le type de mouvement
    if (type.value === 'mutation' && employeDetails) {
        console.log("ato nefa");
        const infosPro = employeDetails.infosProfessionnelles;
        form.setFieldsValue({
        ancienPoste: infosPro?.poste?.nom || '',
        ancienDepartement: infosPro?.poste?.departement?.nom || '',
        ancienSalaire: infosPro?.salaire || 0,
        date: moment()
        });
    }
    // setCurrentStep(2);

    if(type.value === 'changement de poste' && employeDetails){
       console.log("ato nefa lala");
      //  console.log("ancien niveau : ", infosPro?.poste?.niveauHierarchique?.nom);
       const infosPro = employeDetails.infosProfessionnelles;
        form.setFieldsValue({
        ancienPoste: infosPro?.poste?.nom || '',
        ancienDepartement: infosPro?.poste?.departement?.nom || '',
        ancienDepartementId: infosPro?.poste?.departement?.id || '',
        ancienSalaire: infosPro?.salaire || 0,
        date: moment()
        });
    }
    setCurrentStep(2);
      ...
      if(type.value === 'promotion' && employeDetails){
        console.log("ato nefa");
        const infosPro = employeDetails.infosProfessionnelles;
          form.setFieldsValue({
            ancienPoste: infosPro?.poste?.nom || '',
            ...
            ancienDepartementId: infosPro?.poste?.departement?.id || '',
            ancienSalaire: infosPro?.salaire || 0,
            ancienNiveau: infosPro?.poste?.niveauHierarchique?.nom,
            date: moment(), 
            niveauGrade: '+1'
          });
      }
  ...
  const handlePrevStep = () => {
    if (currentStep === 1) {
      ...
      setSelectedTypeMouvement(null);
      form.resetFields(['ancienPoste', 'ancienDepartement', 'ancienSalaire', 'nouveauPoste', 'nouveauDepartement', 'nouveauSalaire', 'motif', 'description']);
    }
    setCurrentStep(currentStep - 1);
  };

  // Gestion de l'édition (ancien flux)
  const handleEditMouvement = (mouvement) => {
    setSelectedMouvement(mouvement);
    setSelectedEmployee(employees.find(emp => emp.id === mouvement.employe_id));
    form.setFieldsValue({
      ...mouvement,
      date: mouvement.date ? moment(mouvement.date) : moment(),
      employe_id: mouvement.employe_id || undefined
    });
    setIsModalVisible(true);
    setCurrentStep(2); // Aller directement à l'étape de détail
  };

  const handleDeleteMouvement = async (mouvementId) => {
    try { 
      const response = await fetch(`http://localhost:8000/api/mouvements/${mouvementId}`, {
        method: 'DELETE',
      });
  ...
  const handleSaveMouvement = async () => {
    try {
      ...
      console.log("Type de mouvement sélectionné:", selectedTypeMouvement);
      
      // Préparer les données pour la demande de mouvement selon le format fourni
      const demandeMouvementData = {
    motif: values.motif,

    dateDemande: values.date
    ...
    employeDemandeur: {
      id: selectedEmployeForMouvement.employe.id
    },
    ...
    // IMPORTANT : doit s'appeler type_mouvement
    typeMouvement: {
      id: selectedTypeMouvement.id
    },

    infosProActuel: {
      id: employeDetails?.infosProfessionnelles?.id
    },
    ...
    infosProPropose: {
      ...
      poste: {
        id: values.nouveauPoste
      },

      // Correction des dates (Moment → String)
      dateDebut: values.dateDebut
        ? moment(values.dateDebut).format('YYYY-MM-DD')
        : null,

      dateFin: values.dateFin
        ? moment(values.dateFin).format('YYYY-MM-DD')
        : null,

      typeContrat: {
        id: employeDetails?.infosProfessionnelles?.typeContrat?.id
      }
    }
  };


    console.log("Données à envoyer : ", demandeMouvementData);

    let response;
    
    if (selectedMouvement) {
      // Modification d'une demande existante (si applicable)

      response = await fetch(`http://localhost:8080/api/mouvements/demande-mouvements/${selectedMouvement.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(demandeMouvementData),
      });
    } else {

      console.log("data demande : ", JSON.stringify(demandeMouvementData));
      // Création d'une nouvelle demande
      response = await fetch('http://localhost:8080/api/mouvements/demande', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    if (response.ok) {
      message.success(
        selectedMouvement 
          ? 'Demande de mouvement modifiée avec succès' 
          : 'Demande de mouvement envoyée avec succès'
      );
      ...
      // Recharger les données si nécessaire (pour afficher les demandes)
      fetchAllData();
    } else {
      const errorData = await response.json();
      message.error(errorData.message || 'Erreur lors de l\'envoi de la demande');
    }
  } catch (error) {
  ...
  const handleNiveauChange = (niveauActuelId) => {
    setSelectedNiveau(niveauActuelId);
    // setSelectedPoste()
  }

  // AJOUT: Gestionnaire pour le changement de département
  const handleDepartementChange = (departementId) => {
    setSelectedDepartement(departementId);
    // setSelectedPoste(null); // Réinitialiser le poste quand on change de département
    
    // Mettre à jour le formulaire
    form.setFieldsValue({
        nouveauPoste: null,
        nouveauDepartement: departementId
    });
  };

  // AJOUT: Gestionnaire pour le changement de poste
  const handlePosteChange = (posteId) => {
    setSelectedPoste(posteId);
    
    // Mettre à jour le formulaire
    form.setFieldsValue({
        nouveauPoste: posteId
    });
  };
  ...
  const stats = {
    ...
    nouveauxEmployes: mouvements.filter(m => m.type === 'arrivee' && moment(m.date).isSame(moment(), 'month')).length
        ...
        // Composant Select pour les postes
    // const SelectPoste = () => {
    ...
    //     setLoading(true);
    //     try {
    //     const response = await fetch('http://localhost:8080/api/postes');
    //     const data = await response.json();
    ...
    // Composant Select pour les départements
    // Remplacer votre SelectDepartement actuel par :
const SelectDepartement = ({ value, onChange }) => {
    ...
    // Remplacer votre SelectPoste actuel par :
const SelectPoste = ({ departementId, value, onChange }) => {
    ...
    const fetchPostesByDepartement = async (departementId) => {
        ...
        // console.log("tonga ato ny id ", departementId);
  ...
  return (
    <div className="container-fluid" style={{ padding: '20px' }}>
            ...
            description={
              <div>
                ...
                <p className="text-muted">
                  ...
                  Commencez par ajouter le premier mouvement.
                </p>
              </div>
            }
          >
      ...
      <Modal
        ...
        open={isModalVisible}
        onOk={currentStep === 2 ? handleSaveMouvement : undefined}
        onCancel={() => {
          setIsModalVisible(false);
          form.resetFields();
          setSelectedMouvement(null);
        ...
        footer={
          currentStep === 2 ? [
            ...
            <Button key="submit" type="primary" loading={loading} onClick={handleSaveMouvement}>
      ...
      >
        <Steps current={currentStep} style={{ marginBottom: 24 }}>
          <Step title="Employé" description="Sélection de l'employé" />
          <Step title="Type" description="Type de mouvement" />
          <Step title="Détails" description="Informations du mouvement" />
        </Steps>
        ...
        <div>
            ...
            {employees.map(employee => {
                const hasInfos = employee.infosProfessionnelles !== null;
                const departement = hasInfos ? employee.infosProfessionnelles.poste?.departement?.nom || 'Non spécifié' : 'Non spécifié';
                const poste = hasInfos ? employee.infosProfessionnelles.poste?.nom || 'Non spécifié' : 'Non spécifié';
                
                return (
                <Option key={employee.employe.id} value={employee.employe.id}>
                    {employee.employe.nom} {employee.employe.prenom} - {departement} - {poste}
                </Option>
                );
            })}
            </Select>
        ...
        {/* Étape 2: Sélection du type de mouvement */}
        {currentStep === 1 && selectedEmployeForMouvement && (
        <div>
            <h4>Sélectionnez le type de mouvement pour {selectedEmployeForMouvement.employe.nom}</h4>
            <Row gutter={[16, 16]}>
            {typesMouvement.map(type => (
                <Col span={8} key={type.id}> {/* Utiliser type.id comme clé */}
                <Card 
                    size="small" 
                    hoverable 
                    onClick={() => handleSelectTypeMouvement(type)}
                    style={{ 
                    textAlign: 'center', 
                    border: selectedTypeMouvement && selectedTypeMouvement.id === type.id ? '2px solid #b053ad' : '1px solid #e1b2db',
                    cursor: 'pointer'
                    }}
                >
        ...
        {/* Étape 3: Détails du mouvement */}
        {currentStep === 2 && selectedEmployeForMouvement && selectedTypeMouvement && (
          <Form form={form} layout="vertical">
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item name="date" label="Date du mouvement" rules={[{ required: true, message: 'Date requise' }]}>
                  <DatePicker style={{ width: '100%' }} format="DD/MM/YYYY" />
                </Form.Item>
              </Col>
              <Col span={12}>
                <Form.Item label="Type de mouvement">
                  <Input value={selectedTypeMouvement.label} disabled /> {/* ⬅️ CORRECTION ICI */}
                </Form.Item>
              </Col>
            </Row>

            {selectedTypeMouvement.value === 'mutation' && (
            <>
                <Divider orientation="left">Situation actuelle</Divider>
                <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name="ancienPoste" label="Poste actuel">
                    <Input disabled />
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item name="ancienDepartement" label="Département actuel">
                    <Input disabled />
                    </Form.Item>
                </Col>
                </Row>
                <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name="ancienSalaire" label="Salaire actuel">
                    <InputNumber style={{ width: '100%' }} min={0} disabled />
                    </Form.Item>
                </Col>
                </Row>

                <Divider orientation="left">Nouvelle situation</Divider>
                <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name="nouveauDepartement" label="Nouveau département">
                    <SelectDepartement 
                ...
                <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name="dateDebut" label="Date début" rules={[{ required: true, message: 'Date requise' }]}>
                      <DatePicker style={{ width: '100%' }} format="DD/MM/YYYY" />
                    </Form.Item>
                </Col>
                 <Col span={12}>
                ...
                <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name="nouveauSalaire" label="Nouveau salaire (Ar)">
                    <InputNumber 
                        style={{ width: '100%' }} 
                        min={0} 
                        placeholder="Nouveau salaire" 
                        formatter={value => `Ar ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                        parser={value => value.replace(/Ar\s?|(,*)/g, '')}
                    />
                    </Form.Item>
                </Col>
                </Row>
            ...
            {selectedTypeMouvement.value === 'changement de poste' &&  (
            <>
                <Divider orientation="left">Situation actuelle</Divider>
                <Row gutter={16}>
                <Col span={12}>
                ...
                <Col span={12}>
                    ...
                    <Form.Item name="ancienDepartementId" hidden>
                        <Input type="hidden" />
                    </Form.Item>
                </Col>
                </Row>
                ...
                <Row gutter={16}>
                 <Col span={12}>
                    <Form.Item 
                      ...
                      rules={[{ required: true, message: 'Le poste est requis' }]}
                    >
                      <SelectPosteWithoutDepartement 
                        // departementId={selectedDepartement}
                        departementId={form.getFieldValue("ancienDepartementId")}
                        onChange={(v) => form.setFieldsValue({ nouveauPoste: v })}
                      />
                    </Form.Item>
                </Col>
                </Row>
            ...
            {selectedTypeMouvement.value === 'promotion' && (
            <> 
                <Divider orientation="left">Situation actuelle</Divider>
                <Row gutter={16}>
                    <Col span={12}>
                ...
                <Row gutter={16}>
                    ...
                    <Col span={12}>
                        <Form.Item name="ancienNiveau" label="Niveau hiérarchique actuel">
                            <Input disabled />
                        </Form.Item>
                    </Col>
                </Row>
                ...
                {/* NOUVEAU : Sélection du niveau hiérarchique */}
                <Row gutter={16}>
                    <Col span={12}>
                        <Form.Item 
                            name="nouveauNiveauId" 
                            label="Nouveau niveau hiérarchique"
                            rules={[{ required: true, message: 'Le niveau hiérarchique est requis' }]}
                        >
                            <SelectNiveauHierarchique 
                                niveauActuelId={form.getFieldValue('ancienNiveauId')}
                                onChange={(value) => {
                                    // Calculer l'écart si besoin
                ...
                <Row gutter={16}>
                    ...
                    <Col span={12}>
                      <Form.Item 
                          ...
                          rules={[{ required: true, message: 'Le poste est requis' }]}
                      >
                          <SelectPosteHierarchique 
                              departementId={selectedDepartement}
                              niveauId={form.getFieldValue("nouveauNiveauId")}  // <-- CHANGEMENT PRINCIPAL
                              onChange={handlePosteChange}
                          />
                      </Form.Item>
                  </Col>
                ...
                {/* Champ caché pour stocker l'ID du niveau actuel */}
                <Form.Item name="ancienNiveauId" hidden>
                    <Input />
                </Form.Item>
            </>
            )}

            <Form.Item name="motif" label="Motif du mouvement">
              <Input placeholder="Raison du mouvement..." />
            </Form.Item>

            <Form.Item name="description" label="Description">
              <TextArea rows={3} placeholder="Description détaillée du mouvement..." />
            </Form.Item>
          </Form>
        )}
      </Modal>
